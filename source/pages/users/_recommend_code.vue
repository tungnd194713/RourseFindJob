<template>
  <form class="signup-body"
        ...
        @submit.prevent="signupFromRecommend"
  >
    ...
    <div class="form_signup form_signup_recommend " :class="{ extraLong: error }">
      <div class="text-content d-flex justify-content-center title_form_signup_recommend">
        {{ $t('general.signup') }}
      </div>
      ...
      <div>
        <div class="code">
          <div class="text-email">{{ $t('recommend_page.code_recommend') }}<p>*</p></div>
          <img class="ic_mail" src="~/assets/images/ic_mail.svg" alt="">
          <div class="line"></div>
          <input
            v-model.trim="$route.params.recommend_code"
            type="text" class="input-email"
            disabled
          >
        </div>
      </div>
      ...
      <div>
        <div class="firstname">
          ...
          <input v-model="user.name"
                 ...
                 @keydown.enter.prevent="signupFromRecommend"
          >
        </div>
      ...
      <div class="signCon">
        ...
        <button class="signup" type="submit">{{ $t('signup_page.signup') }}</button>
      </div>
    </div>
  </form>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      async signupFromRecommend() {
        this.$v.user.$touch()
        this.$v.acceptTerms.$touch()

        if (!this.$v.$invalid && this.acceptTerms === true) {
          try {
            await this.$repositories.accounts.signUp({
              ...this.user,
              recommend_user_code: this.$route.params.recommend_code
            })
              .then((res) => {
                if (res.status === 200) {
